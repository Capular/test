            ...
            const checkStatus = async () => {
                try {
                    ...
                    if (data.status === 'success' || (data.result && data.result.status === 'success')) {
                        ...
                        /* @ts-ignore */
                        const q = query(collection(db, "transactions"), where("gatewayOrderId", "==", txnId));
                        /* @ts-ignore */
                        ...
                        if (!querySnapshot.empty) {
                            ...
                            console.log("Client-side fallback: Transaction updated to success");
                        }
                    }
                } catch (err) {
                                ...
                                <button
                                    onClick={async () => {
                                        try {
                                            ...
                                            const data = await res.json();
                                            if (data.status === 'success' || (data.result && data.result.status === 'success')) {
                                                // Trigger the useEffect logic or reload
