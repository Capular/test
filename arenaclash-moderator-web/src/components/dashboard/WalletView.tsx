...
function TransactionDetailDialog({ transaction, onClose, getTransactionStyle }: TransactionDetailDialogProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Overlay with background and blur - GSAP animates this */}
            <div
                ref={overlayRef}
            ...
            >
                <div
                    ref={dialogRef}
                    className="relative w-full bg-card/95 backdrop-blur-xl border border-border/50 rounded-xl p-6 shadow-2xl pointer-events-auto overflow-hidden ring-1 ring-border/20"
                    style={{
                        transformStyle: 'preserve-3d',
                        willChange: 'transform, opacity',
                        // Removed hardcoded background/boxShadow
                    }}
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            // Improved check for SUCCESS or TXN_SUCCESS from gateway
                                            const statusVal = String(data.status || '').toUpperCase();
                                            const isSuccess = statusVal === 'SUCCESS' || statusVal === 'TXN_SUCCESS' || 
                                                              (data.result && (String(data.result.status).toUpperCase() === 'SUCCESS' || String(data.result.status).toUpperCase() === 'TXN_SUCCESS'));

                                            if (isSuccess) {
                                                // Success: Server has updated the DB.
                                                // Real-time listener will update the UI automatically.
                                                toast.success(`Verified: ${data.message || "Payment Successful"}`, { id: toastId });
                                                handleClose();
                                            } else {
                                                // Show simplified, color-coded message based on gateway status
                                                const status = data.gatewayStatus;
                                                const msg = data.error || data.message || "Payment not successful";

                                                if (status === 'PENDING' || status === 'CREATED') {
                                                    // Yellow/Neutral custom toast for Pending
                                                    toast(msg, { 
                                                        id: toastId,
                                                        icon: '⏳',
                                                        style: { border: '1px solid #EAB308', color: '#EAB308' } // Yellow-500 equivalent
                                                    });
                                                } else {
                                                    // Red for Failure
...
const { user, loading: authLoading } = useAuth();
    ...
    // Standardized page reveal manually handled to support auth loading state
    const containerRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        if (!authLoading && user && containerRef.current) {
    ...
    const filteredTransactions = useMemo(() => {
        if (activeTab === 'deposits') {
            return allTransactions.filter(tx => tx.type === 'deposit' || tx.type === 'prize');
        }
        if (activeTab === 'withdrawals') {
            return allTransactions.filter(tx => tx.type === 'withdrawal' || tx.type === 'entry');
        }
        return [];
    ...
    useEffect(() => {
        ...
        const txUnsub = onSnapshot(q, (snapshot) => {
            ...
            // Auto-Verify Old Pending Transactions (> 5 minutes)
            const now = Date.now();
            const fiveMinutesAgo = now - (5 * 60 * 1000);
            
            const pendingAndOld = txs.filter(t => {
                if(t.status !== 'pending') return false;
                const txTime = t.timestamp?.toDate ? t.timestamp.toDate().getTime() : 0;
                // Verify if older than 5 mins
                return txTime > 0 && txTime < fiveMinutesAgo;
            });

            if (pendingAndOld.length > 0) {
                console.log(`Found ${pendingAndOld.length} stuck pending transactions. Verifying...`);
                // Process singly to avoid slamming API
                pendingAndOld.forEach(async (tx) => {
                    try {
                        const orderId = tx.gatewayOrderId || tx.orderId;
                        if (!orderId) {
                            // If no ID, it's a zombie record, maybe mark failed?
                            return;
                        }
                        ...
                        await fetch('/api/payment/status', {
                         ...
                         console.error("Auto-verify failed for", tx.id, e);
                    }
                });
            }
        }, (error) => {
    ...
    // 1. Check Auth Loading FIRST - Show FULL PAGE skeleton to avoid layout shift
    if (authLoading) {
        return (
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="bg-card border border-border rounded-xl p-6">
                        ...
                        <div className="flex gap-3">
                            <Button
                                ...
                                className="flex-1 bg-primary text-primary-foreground h-auto py-2.5 font-semibold text-sm hover:bg-primary/90 transition-colors"
                            >
                                Add Funds
                            </Button>
                            <Button 
                                className="flex-1 bg-muted text-foreground h-auto py-2.5 font-semibold text-sm hover:bg-muted/80 transition-colors"
                    ...
                    <div className="bg-card border border-border rounded-xl p-4">
                        ...
                        {/* Tabs */}
                        <div className="flex gap-2 mb-4 p-1 bg-muted/20 border border-border/50 rounded-lg w-full max-w-sm mx-auto">
