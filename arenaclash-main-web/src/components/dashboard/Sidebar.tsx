...
import { Gamepad2, Wallet, Settings, ChevronDown, User, LogOut, ClipboardList, MessageSquare, HelpCircle, Info, Zap, ExternalLink } from "lucide-react";
...
export default function Sidebar({ onLoginClick }: SidebarProps) {
    ...
    const enabledMainMenuItems = mainMenuItems.filter((item) => {
        if (item.id === "tournaments") return flags.tournamentsEnabled;
        if (item.id === "wallet") return flags.walletEnabled;
        return true;
    });

    const enabledResourceMenuItems = resourceMenuItems.filter((item) => {
        if (item.id === "support" || item.id === "help") return flags.supportEnabled;
        return true;
    });
    ...
    useEffect(() => {
        navItemsRef.current.forEach((btn, i) => {
            if (!btn) return;
            const item = i < enabledMainMenuItems.length ? enabledMainMenuItems[i] : enabledResourceMenuItems[i - enabledMainMenuItems.length];
    ...
    const renderNavItem = (item: typeof enabledMainMenuItems[0], index: number) => {
        const active = isActive(item.path);
        return (
            <Button
                key={item.id}
                ...
                <nav className="flex-1 overflow-y-auto px-3 pt-4 space-y-1 custom-scrollbar">
                    ...
                    <div className="mb-4">
                        ...
                        <div className="space-y-1">
                            {enabledMainMenuItems.map((item, i) => renderNavItem(item, i))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            {enabledResourceMenuItems.map((item, i) => renderNavItem(item, enabledMainMenuItems.length + i))}
                        </div>
                    </div>
                </nav>
            </div>
