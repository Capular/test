...
export async function middleware(req: NextRequest) {
  ...
  if (!flags.tournamentsEnabled) {
    if (
      ...
      return deny(req, "Tournament feature is disabled for this tenant.");
    }
  }
  ...
  if (!flags.walletEnabled) {
    if (
      ...
      pathname.startsWith("/api/zapupi-webhook")
    ) {
  ...
  if (!flags.supportEnabled) {
    if (pathname.startsWith("/support") || pathname.startsWith("/help")) {
      return deny(req, "Support features are disabled for this tenant.");
    }
  }
